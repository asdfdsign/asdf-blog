...
플래그를 바꿀 때 지킬 것 — 볼트는 되돌릴 수 없는 개인 노트다.
- 볼트에서 손대도 되는 곳은 `블로그/` 폴더뿐이다. 나머지는 샌드박스가 막는다 (공통 설정의 `sandbox.filesystem.allowWrite` 에 `블로그/` 만 열려 있다).
- Write/Edit 도구는 샌드박스에 막히므로 **Bash + python** 으로 바꾼다. 읽어서 변수에 담고 → 바꾸고 → 길이·내용을 확인한 뒤 → **한 번만** 쓴다. 같은 경로를 쓰기로 연 채 그 안에서 읽지 않는다 (2026-09-18 에 그 실수로 파일이 0바이트가 됐다).
- 권한·샌드박스 시험은 `블로그/__perm-test.md` 같은 임시 파일로 하고 지운다. 기존 노트를 시험 대상으로 삼지 않는다.
