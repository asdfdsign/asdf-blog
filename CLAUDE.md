...
4. Embed — `content/apps.json`에 항목(`slug` `title` `date` `summary`)을 추가한다. 블로그의 「웹앱」 카테고리(`#/cat/apps`)에 카드가 생기고, 누르면 `apps/{slug}/`로 이동한다. 브라우저에서 확인하고 깃 커밋한다.
...
- 모바일에서도 사용할 수 있어야 한다.
- **색은 블로그 팔레트를 따른다.** `css/style.css`의 `:root` / `[data-theme="dark"]` 변수 블록을 앱 `style.css`에 그대로 복사해 쓰고, 컴포넌트 CSS에는 색상값을 직접 쓰지 않는다. 블로그에 없는 의미(예: 포모도로의 휴식 `--rest`)가 필요할 때만 변수를 추가하되, 양쪽 테마에 정의하고 대비 4.5:1을 실측한다. 테마는 블로그와 같은 `localStorage` 키 `theme`과 `data-theme`으로 연동한다. 블로그 팔레트를 바꾸면 `apps/*/style.css`도 같이 고친다.
- **사용법 안내를 넣는다.** 먼저 안내 없이도 쓸 수 있게 만들고, 그 위에 접이식 `<details>` 「사용법」 하나를 둔다 — 조작 방법과 키보드 단축키를 짧게. 임베드(iframe) 모드에서는 숨긴다. 화면 위에 설명 문단을 상시로 얹지 않는다.
...
- **웹앱 목록은 `content/apps.json`이 진실이다.** `posts.json`과 같은 원리 — 앱 폴더만 만들고 항목을 안 넣으면 블로그에 없는 앱이다. 순서는 파일에 적힌 대로(최신이 위). 「웹앱」은 글이 아니라 카테고리 바의 항목이며 `전체`에는 세지 않는다. 앱은 `data-embed`로 iframe 안임을 감지하는 임베드 모드를 유지한다(지금은 카드에 iframe 을 쓰지 않지만 앱 폴더가 자체 완결이라 그대로 둔다).
