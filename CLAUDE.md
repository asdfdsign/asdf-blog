...
- **웹앱 카드는 `index.html`의 `tpl-list` 안 `.apps` 절에 정적으로 넣는다.** 제목·설명·`새 탭에서 열기` 링크·`loading="lazy"` iframe(`./apps/{앱이름}/`, 높이 480). 전체 목록(`#/`)에서만 보이고 카테고리·주제로 거르면 `app.js`가 절을 떼어 낸다. 앱은 `data-embed`로 iframe 안임을 스스로 감지해 헤더를 숨기고 보드를 위로 올린다(`apps/sudoku/theme.js` 참고).
